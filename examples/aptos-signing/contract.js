export async function handle(state, action) {
  ...
  const aptos_molecule_endpoint = state.aptos_molecule_endpoint;
  ...
  if (input.function === "register") {
    ...
    state.names[res.address] = `${name.trim()}.aptos`;
  ...
  async function _moleculeSignatureVerification(caller, message, signature) {
    try {
      const isValid = await EXM.deterministicFetch(
        `${aptos_molecule_endpoint}/aptos-auth/${caller}/${message}/${signature}`
      );
